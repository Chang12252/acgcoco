...
public class BaseController {
     ...
     * 把用户信息存入redis
     */
    public Cookie putLoginUser(UserBO user, HttpServletResponse response) {
        String uuid = UUID.randomUUID().toString();
        this.putSession(createKey(user.getId() + uuid, CURRENT_LOGIN_USER), user);
        return this.setCookie(response, CURRENT_LOGIN_USER, createKey(user.getId() + uuid, CURRENT_LOGIN_USER));
    }
     ...
     * @param value
     */
    public Cookie setCookie(HttpServletResponse response, String key, String value) {
    ...
    public String getCookie(HttpServletRequest request, String key) {
        ...
        for(int i=0;i<split.length;i++){
            for(String sp:split){
                if(sp.contains(key)){
                    return sp.substring(sp.indexOf("=")+1);
                }
            }
        }
        return null;
